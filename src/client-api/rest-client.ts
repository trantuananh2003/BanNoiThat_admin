...
class RestClient {
    ...
    // Xác thực tài khoản
    async authentication(strategy: string, email: string, password: string): Promise<any> {
    ...
    async reAuthentication(): Promise<any> {
        ...
        try {
            const response = await this.axiosInstance.post(`/${this.path}/re-auth`, { token: this.authToken });
            if (response.data?.token) {
                this.authToken = response.data.token;
                if (this.authToken) {
                    localStorage.setItem('userToken', this.authToken);
    ...
    async get<T>(objectId: string): Promise<T> {
        try {
            const response = await this.axiosInstance.get<T>(`/${this.path}/${objectId}`);
            return response.data;
        } catch (error) {
    ...
    async patch<T>(objectId: string, data: any): Promise<T> {
        try {
            const isFormData = data instanceof FormData;
            const response = await this.axiosInstance.patch<T>(`/${this.path}/${objectId}`, data, {
                headers: { 'Content-Type': isFormData ? 'multipart/form-data' : 'application/json' },
            });
            return response.data;
        } catch (error) {
    ...
    async remove<T>(objectId: string): Promise<T> {
        try {
            const response = await this.axiosInstance.delete<T>(`/${this.path}/${objectId}`);
            return response.data;
        } catch (error) {
    ...
    async signup<T>({ username, email, password }: { username: string; email: string; password: string }): Promise<T> {
        try {
            const response = await this.axiosInstance.post<T>('/services/sign-up', { username, email, password });
            return response.data;
        } catch (error) {
