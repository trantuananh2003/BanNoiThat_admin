...
const OrderPage = () => {
  ...
  const LoadOrders = async () => {
    ...
    try {
      ...
      const response: ApiResponse = await clientAPI
        ...
        .find(`orderStatus=${activeTab}&latestStatusDay=${selectedDate?.toLocaleDateString()}`);
  ...
  const handleCreateOrderGHN = async (id: string) => {
    const result = await Swal.fire({
      title: "Xác nhận",
      text: "Bạn có chắc chắn muốn tạo đơn hàng GHN?",
      icon: "question",
      showCancelButton: true,
    ...
    try {
      ...
      await Swal.fire("Thành công", "Tạo đơn hàng GHN thành công!", "success");
      LoadOrders();
    } catch (error: any) {
      Swal.fire(
        "Lỗi",
